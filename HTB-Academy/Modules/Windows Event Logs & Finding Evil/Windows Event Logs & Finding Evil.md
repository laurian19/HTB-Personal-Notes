...
![[Windows Event Logs & Finding Evil-1774867792007.webp]]

![[Windows Event Logs & Finding Evil-1774867796172.webp]]
...
![[Windows Event Logs & Finding Evil-1774867803947.webp]]
...
![[Windows Event Logs & Finding Evil-1774867816988.webp]]
...
![[Windows Event Logs & Finding Evil-1774867824677.webp]]
...
![[Windows Event Logs & Finding Evil-1774867840451.webp]]
...
![[Windows Event Logs & Finding Evil-1774867852378.webp]]
...
![[Windows Event Logs & Finding Evil-1774867861450.webp]]
...
![[Windows Event Logs & Finding Evil-1774867877985.webp]]
...
![[Windows Event Logs & Finding Evil-1774867886975.webp]]
...
![[Windows Event Logs & Finding Evil-1774867894357.webp]]
...
![[Windows Event Logs & Finding Evil-1774867903317.webp]]
...
The filter we used is:

```xml
<QueryList>
  <Query Id="0" Path="Security">
    <Select Path="Security">*[System[(EventID=4907)] and EventData[Data[@Name='SubjectLogonId']='0x3e7'] and EventData[Data[@Name='ObjectName']='C:\Windows\Microsoft.NET\Framework64\v4.0.30319\WPF\wpfgfx_v0400.dll']]
	</Select>
  </Query>
</QueryList>
```

![[Windows Event Logs & Finding Evil-1774880138217.webp]]
...
![[Windows Event Logs & Finding Evil-1774880247156.webp]]
...
![[Windows Event Logs & Finding Evil-1774880277032.webp]]
...
![[Windows Event Logs & Finding Evil-1774880285363.webp]]
...
![[Windows Event Logs & Finding Evil-1774880299027.webp]]
...
![[Windows Event Logs & Finding Evil-1774880307327.webp]]

Let's now see how a Sysmon event ID 7 looks like.

![[Windows Event Logs & Finding Evil-1774880317734.webp]]
...
![[Windows Event Logs & Finding Evil-1774880328180.webp]]
...
![[Windows Event Logs & Finding Evil-1774880339869.webp]]
...
![[Windows Event Logs & Finding Evil-1774880373315.webp]]
...
![[Windows Event Logs & Finding Evil-1774880390657.webp]]
...
![[Windows Event Logs & Finding Evil-1774880404780.webp]]
...
![[Windows Event Logs & Finding Evil-1774880411708.webp]]
...
![[Windows Event Logs & Finding Evil-1774880424199.webp]]

After the injection, we observe that "spoolsv.exe" transitions from an unmanaged to a managed state.

![[Windows Event Logs & Finding Evil-1774880446657.webp]]
...
![[Windows Event Logs & Finding Evil-1774880455995.webp]]

![[Windows Event Logs & Finding Evil-1774880461190.webp]]
...
![[Windows Event Logs & Finding Evil-1774880494594.webp]]

![[Windows Event Logs & Finding Evil-1774880499123.webp]]
...
3. Replicate the Credential Dumping attack described in this section and provide the NTLM hash of the Administrator user as your answer. "C:\Tools\Sysmon" and "C:\Tools\Mimikatz" on the spawned target contain everything you need.m
...
![[Windows Event Logs & Finding Evil-1774959406169.webp]]
...
![[Windows Event Logs & Finding Evil-1774959558164.webp]]

![[Windows Event Logs & Finding Evil-1774959563791.webp]]

ETW Provider metadata can also be viewed through the [EtwExplorer project](https://github.com/zodiacon/EtwExplorer).

![[Windows Event Logs & Finding Evil-1774959572215.webp]]
...
![[Windows Event Logs & Finding Evil-1774961623344.webp]]
...
![[Windows Event Logs & Finding Evil-1774961633283.webp]]
...
![[Windows Event Logs & Finding Evil-1774961655490.webp]]
...
![[Windows Event Logs & Finding Evil-1774961671505.webp]]
...
![[Windows Event Logs & Finding Evil-1774961690608.webp]]
...
![[Windows Event Logs & Finding Evil-1774961732715.webp]]

![[Windows Event Logs & Finding Evil-1774961736356.webp]]
...
![[Windows Event Logs & Finding Evil-1774961760534.webp]]
...
![[Windows Event Logs & Finding Evil-1774966073837.webp]]
...
![[Windows Event Logs & Finding Evil-1774966158097.webp]]
...
We used the following command within PowerShell:

```
PS C:\Users\Administrator> Get-WinEvent -FilterHashTable @{Path='C:\Tools\chainsaw\EVTX-ATTACK-SAMPLES\Lateral Movement\*'; ID=5142} | Select-Object TimeCreated, ID, Message | Format-List
...
Get-WinEvent -Path 'C:\Tools\chainsaw\EVTX-ATTACK-SAMPLES\Lateral Movement\*' -FilterXPath "*[System[(EventID=5142)] and EventData[Data[@Name='ShareName']='\\*\ADMIN$']]" | Select-Object TimeCreated, Id, Message | Format-List
```

We can also use the following command for a more specific match:

```
Get-WinEvent -Path 'C:\Tools\chainsaw\EVTX-ATTACK-SAMPLES\Lateral Movement\*' -FilterXPath "*[System[(EventID=5142)] and EventData[Data[@Name='ShareName']='\\*\PRINT']]" | Select-Object TimeCreated, Id, Message | Format-List
```
...
We use the folGet-WinEvent -Path 'C:\Logs\PowershellExec\*' | Where-Object{$_.ID -like "8"} | Where-Object{$_.Message -like "*Calculator.exe*"} | Select-Object TimeCreated, ID, ProviderName, LevelDisplayName, Message
lowing XML filter:

```xml
<QueryList>
  <Query Id="0" Path="file://C:\Logs\DLLHijack\DLLHijack.evtx">
    <Select Path="file://C:\Logs\DLLHijack\DLLHijack.evtx">*[System[(EventID=7)] and
EventData[Data[@Name='Signed']='false']]</Select>
  </Query>
</QueryList>
```

We found two events, but the one below is the one we are looking for, because: 
- `DismCore.dll` is originally Microsoft-signed, but this one is not.
- `DismCore.dll` and `Dism.exe` are originally located under within `C:\Windows\system32\Dism\DismCore.dll`, but these ones are in a writable location.

![[Windows Event Logs & Finding Evil-1774971515680.webp]]

We can also use the following command in PowerShell:

```
PS C:\Users\Administrator> Get-WinEvent -Path 'C:\Logs\DLLHijack\*' -FilterXPath "*[System[(EventID=7)] and EventData[Data[@Name='Signed']='false']]" | Select-Object TimeCreated, Id, Message | Format-List
...
We filter for Event ID 7. 

We look for the keyword `clr.dll`. Only two executables load this DLL and one of them is `PowerShell`, so it is not the one we are looking for.

![[Windows Event Logs & Finding Evil-1774972246688.webp]]

```
PS C:\Users\Administrator> Get-WinEvent -Path 'C:\Logs\PowershellExec\*' -FilterXPath "*[System[(EventID=7)] and EventData[Data[@Name='ImageLoaded']='C:\Windows\Microsoft.NET\Framework64\v4.0.30319\clr.dll']]" | Select-Object TimeCreated, Id, Message | Format-List
...
Message     : Image loaded:
              RuleName: -
              UtcTime: 2022-04-28 01:58:45.687
...
```
Get-WinEvent -Path 'C:\Logs\PowershellExec\*' | Where-Object{$_.ID -like "1"} | Where-Object{$_.Message -like "*Calculator.exe*"} | Select-Object TimeCreated, ID, ProviderName, LevelDisplayName, Message
```

We use Sysmon Event ID 8:

![[Windows Event Logs & Finding Evil-1774978861376.webp]]

So Event ID 8 is telling us:
- **SourceImage** = process creating the remote thread
- **TargetImage** = process receiving that thread

And we use the following filter (note the process ID we filter by):

```xml
<QueryList>
  <Query Id="0" Path="file://C:\Logs\PowershellExec\PowershellExec.evtx">
    <Select Path="file://C:\Logs\PowershellExec\PowershellExec.evtx">*[System[(EventID=8)] and EventData[Data[@Name='TargetProcessId']='3776']]</Select>
  </Query>
</QueryList>
```

The results show two events, and in both of them we see `rundll32.exe` as the `SourceImage`:

![[Windows Event Logs & Finding Evil-1774978941472.webp]]

![[Windows Event Logs & Finding Evil-1774978949822.webp]]

We can also use the following PowerShell command:

```
PS C:\Users\Administrator> Get-WinEvent -Path 'C:\Logs\PowershellExec\*' -FilterXPath "*[System[(EventID=8)] and EventData[Data[@Name='TargetProcessId']='3776']]" | Select-Object TimeCreated, Id, Message | Format-List
...
```
PS C:\Users\Administrator> Get-WinEvent -Path 'C:\Logs\Dump\*' -FilterXPath "*[System[(EventID=10)] and EventData[Data[@Name='TargetImage']='C:\Windows\system32\lsass.exe']]" | Where-Object {$_.Properties[5].Value -like "*Downloads*"} | Select-Object TimeCreated, Id, Message | Format-List
...
Message     : Process accessed:
              RuleName: -
              UtcTime: 2022-04-28 02:08:56.843
              ...
              TargetProcessGUID: {67e39d39-f738-6269-6303-000000000300}
              TargetProcessId: 6076
              TargetImage: C:\Windows\system32\lsass.exe
              GrantedAccess: 0x1400
              CallTrace: C:\Windows\SYSTEM32\ntdll.dll+9d234|C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe+9373b|C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe+95a1b|C:\Users\waldo\Downloads\processhacker-3.0.48
              01-bin\64bit\ProcessHacker.exe+175751|C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe+10952b|C:\Windows\System32\KERNEL32.DLL+17034|C:\Windows\SYSTEM32\ntdll.dll+52651
              SourceUser: DESKTOP-R4PEEIF\waldo
...
Message     : Process accessed:
              RuleName: -
              UtcTime: 2022-04-28 02:08:56.608
              SourceProcessGUID: {67e39d39-f72f-6269-6203-000000000300}
              SourceProcessId: 5560
              SourceThreadId: 8152
              SourceImage: C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe
              ...
              CallTrace: C:\Windows\SYSTEM32\ntdll.dll+9d714|C:\Windows\SYSTEM32\ntdll.dll+113c90|C:\Windows\System32\KERNELBASE.dll+ddc4e|C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe+5035e|C:\Users\waldo\Downloads\processhacker-3.0.4801-b
              in\64bit\ProcessHacker.exe+10952b|C:\Windows\System32\KERNEL32.DLL+17034|C:\Windows\SYSTEM32\ntdll.dll+52651
              SourceUser: DESKTOP-R4PEEIF\waldo
              TargetUser: NT AUTHORITY\SYSTEM
...
Message     : Process accessed:
              ...
              TargetImage: C:\Windows\system32\lsass.exe
              GrantedAccess: 0x1FFFFF
              CallTrace: C:\Windows\SYSTEM32\ntdll.dll+9d234|C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe+4fbe6|C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe+48875|C:\Users\waldo\Downloads\processhacker-3.0.48
              01-bin\64bit\ProcessHacker.exe+46395|C:\Windows\System32\USER32.dll+e858|C:\Windows\System32\USER32.dll+e4ee|C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\plugins\ToolStatus.dll+782f|C:\Windows\System32\USER32.dll+e858|C:\Windows\System32\USER3
              2.dll+e3dc|C:\Windows\System32\USER32.dll+20bc3|C:\Windows\SYSTEM32\ntdll.dll+a0ba4|UNKNOWN(FFFFF802436B13F5)|UNKNOWN(FFFFFA21FFF1DB1D)|UNKNOWN(FFFFFA21FFF4A254)|UNKNOWN(FFFFFA21FFF49E42)|UNKNOWN(FFFFFA21FFF49BB4)|UNKNOWN(FFFFFA21FFFA1EDC)|UNKNOWN(FFFFFA21
              FFF9420D)|UNKNOWN(FFFFFA21FEF26EB5)|UNKNOWN(FFFFF802434092B5)|C:\Windows\System32\win32u.dll+1124|C:\Windows\System32\USER32.dll+df33|C:\Windows\System32\USER32.dll+d68a|C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe+6a136
              SourceUser: DESKTOP-R4PEEIF\waldo
              TargetUser: NT AUTHORITY\SYSTEM

TimeCreated : 4/27/2022 7:08:52 PM
Id          : 10
Message     : Process accessed:
              RuleName: -
              UtcTime: 2022-04-28 02:08:52.218
              SourceProcessGUID: {67e39d39-f72f-6269-6203-000000000300}
              SourceProcessId: 5560
              SourceThreadId: 7492
              SourceImage: C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe
              ...
              CallTrace: C:\Windows\SYSTEM32\ntdll.dll+9d234|C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe+67ff7|C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe+688e5|C:\Users\waldo\Downloads\processhacker-3.0.48
              01-bin\64bit\ProcessHacker.exe+6a045|C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe+a5cae|C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe+16b5d6|C:\Windows\System32\USER32.dll+e858|C:\Windows\System3
              2\USER32.dll+e3dc|C:\Windows\System32\USER32.dll+20bc3|C:\Windows\SYSTEM32\ntdll.dll+a0ba4|UNKNOWN(FFFFF802436B13F5)|UNKNOWN(FFFFFA21FFF1DB1D)|UNKNOWN(FFFFFA21FFF4A254)|UNKNOWN(FFFFFA21FFF49E42)|UNKNOWN(FFFFFA21FFF49BB4)|UNKNOWN(FFFFFA21FFFA1EDC)|UNKNOWN(F
              FFFFA21FFF9420D)|UNKNOWN(FFFFFA21FEF26EB5)|UNKNOWN(FFFFF802434092B5)|C:\Windows\System32\win32u.dll+1124|C:\Windows\System32\USER32.dll+df33|C:\Windows\System32\USER32.dll+d68a|C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe+16d
              172|C:\Users\waldo\Downloads\processhacker-3.0.4801-bin\64bit\ProcessHacker.exe+16b8e5
              SourceUser: DESKTOP-R4PEEIF\waldo
              TargetUser: NT AUTHORITY\SYSTEM
...
We looked at `SecurityEvents.evtx` and filter by Event ID 4624. We found only two events that took place after the LSASS dump from above, and neither look suspicious:

![[Windows Event Logs & Finding Evil-1774980661585.webp]]

![[Windows Event Logs & Finding Evil-1774980672560.webp]]

We can use the following PowerShell command as well:

```
$dumpTime = Get-Date '2022-04-27 18:39:00'
Get-WinEvent -Path 'C:\Logs\Dump\*' | Where-Object {$_.Id -eq 4624 -and $_.TimeCreated -gt $dumpTime} | Select-Object TimeCreated, Id, Message |
Format-List
```
...
```
PS C:\Users\Administrator> Get-WinEvent -Path 'C:\Logs\StrangePPID\*' -FilterXPath "*[System[(EventID=1)]]" | Select-Object TimeCreated, Id, Message | Format-List
...
Message     : Process Create:
              RuleName: -
              UtcTime: 2022-04-28 02:18:06.631
              ProcessGuid: {67e39d39-f95e-6269-8403-000000000300}
              ProcessId: 4224
              ...
              TerminalSessionId: 1
              IntegrityLevel: Medium
              Hashes: SHA1=52A7274A0B4F9493632060FE25993A2EF24FE827,MD5=0D698AF330FD17BEE3BF90011D49251D,SHA256=3C1C6D813D2B031D988204155FC198FE4F32FF56C05DABBCFCD5486131F4FB9D,IMPHASH=C21B297AEB85CEF1BCCE8D72106BBDD0
              ParentProcessGuid: {67e39d39-f95e-6269-8303-000000000300}
              ParentProcessId: 472
              ParentImage: C:\Windows\System32\cmd.exe
              ParentCommandLine: cmd.exe /c whoami
...
TimeCreated : 4/27/2022 7:17:25 PM
Id          : 1
Message     : Process Create:
              RuleName: -
              UtcTime: 2022-04-28 02:17:25.830
              ...
              TerminalSessionId: 1
              IntegrityLevel: Medium
              Hashes: SHA1=33DEFCDF737C91F0AFFEBC1592AD3626E2FF7A7D,MD5=1C322CDA56B7F497F81FB11B762DA8FF,SHA256=78E2A8F50342F7E02EE354AF29A7B25862DF40F16FD7344D034DCF22FFC82F7A,IMPHASH=A8411DCFB6906C782549D77E5571DC7E
...
The suspicious part of the chain is this:

- `explorer.exe` → `WerFault.exe`
- `WerFault.exe` → `cmd.exe /c whoami`
- `cmd.exe` → `whoami.exe`

The key anomaly is: **`WerFault.exe` spawning `cmd.exe /c whoami`**. `WerFault.exe` normally handles crash/error reporting behavior. It is **not** expected to launch command shells to run things like `whoami`.
